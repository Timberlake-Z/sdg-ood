...
# -> [(-max(softmax))] indicates inconfidence level (-confidence level)
...
def train(epoch, gamma):
    ...
    for batch_idx, (in_set, out_set) in enumerate(zip(train_loader_in, train_loader_out)):
        ...
        # optimize the p for each point in each iteration (parallel)
        # each point search args.iter times
